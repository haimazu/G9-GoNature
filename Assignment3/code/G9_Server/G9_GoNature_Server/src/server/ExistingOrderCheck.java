...
public class ExistingOrderCheck {
	...
	//input: array list of object contains:
	//			[0] -> String checkIfPending
	//			[1] -> orderNumber
	//
	//output: NONE
	//send to client: true if exist in pendingwaitlist false if not
	public static void checkIfPending(ArrayList<Object> recived, ConnectionToClient client) {
		ArrayList<Object> answer = new ArrayList<Object>();
		ArrayList<String> query = new ArrayList<String>();
		answer.add(recived.get(0));
		...
		query.add("*"); // columns to select from
		query.add("WHERE orderNumber='" + order + "'");
		query.add("2");
		ArrayList<ArrayList<String>> queryData = MySQLConnection.select(query);
		answer.add(!queryData.isEmpty());
		EchoServer.sendToMyClient(answer, client);
	}
}
